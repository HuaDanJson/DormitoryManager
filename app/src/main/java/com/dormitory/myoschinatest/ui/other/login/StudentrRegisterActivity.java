...
public class StudentrRegisterActivity extends AppCompatActivity {
    ...
    public void setRegisterClick() {
        ...
        if (TextUtils.isEmpty(userName) || TextUtils.isEmpty(passWord1) || TextUtils.isEmpty(passWord2) || TextUtils.isEmpty(name) || TextUtils.isEmpty(old) || TextUtils.isEmpty(tellPhone) || TextUtils.isEmpty(mail)) {
            Toast.makeText(StudentrRegisterActivity.this, "请输入完整信息后再注册", Toast.LENGTH_SHORT).show();
        } else {
            if (passWord1.equals(passWord2)) {
                ...
                dbUserInfoBean.setTypeOfWork("学生");
                dbUserInfoBean.setTypeOfWorkManager(1);
                ...
                dbUserInfoBean.signUp(new SaveListener<DBTaskManagerUserInfoBean>() {
                    ...
                    public void done(DBTaskManagerUserInfoBean dbTaskManagerUserInfoBean, BmobException e) {
                        if (e == null) {
                            ...
                            Toast.makeText(StudentrRegisterActivity.this, "恭喜您，注册成功", Toast.LENGTH_SHORT).show();
                            StudentrRegisterActivity.this.finish();
                        } else {
                            Toast.makeText(StudentrRegisterActivity.this, "注册失败 ：" + e, Toast.LENGTH_SHORT).show();
                        }
                    }
                });

            } else {
                Toast.makeText(StudentrRegisterActivity.this, "两次密码输入不一致", Toast.LENGTH_SHORT).show();
            }
        }
    }
